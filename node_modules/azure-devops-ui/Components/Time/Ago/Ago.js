...
var Ago = /** @class */ (function (_super) {
    tslib_1.__extends(Ago, _super);
    function Ago(props) {
        var _this = _super.call(this, props) || this;
        _this.getTimeString = function () {
            return DateUtil.ago(_this.props.date, _this.props.format);
        };
        /**
    ...
    Ago.prototype.render = function () {
        return (React.createElement(Time, { className: this.props.className, dateTime: this.props.date, getNextInterval: this.getNextInterval, getTimeString: this.getTimeString, tooltipProps: this.state.tooltipProps }));
    };
    /**
     * Returns time in milliseconds for next refresh.
     *
    ...
    Ago.agoNextInterval = function (date, format, now) {
        ...
        else if (diff < 59 * DateUtil.minute && format === 0 /* Compact */) {
            interval = DateUtil.minute - (diff % DateUtil.minute);
        }
        else if (now.toDateString() === date.toDateString() && format === 0 /* Compact */) {
            interval = DateUtil.hour - (diff % DateUtil.hour);
        }
        else if ((format === 1 /* Extended */ && now.toDateString() === date.toDateString()) ||

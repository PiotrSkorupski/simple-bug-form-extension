...
var ResponsiveLayout = /** @class */ (function (_super) {
    tslib_1.__extends(ResponsiveLayout, _super);
            ...
            if (componentElement && componentElement.children.length) {
                ...
                for (var componentIndex = 0; componentIndex < componentElement.children.length; componentIndex++) {
                    ...
                    if (_this.props.ignoredChildren && _this.props.ignoredChildren.indexOf(componentIndex) >= 0) {
                        ignoredSize += _this.props.orientation === 1 /* Vertical */ ? clientRect.height : clientRect.width;
                    }
                }
                ...
                if (_this.props.orientation === 1 /* Vertical */) {
                    renderedSize = _this.childDetails[_this.childDetails.length - 1].clientRect.bottom - _this.childDetails[0].clientRect.top;
                }
                else {
                    renderedSize = _this.childDetails[_this.childDetails.length - 1].clientRect.right - _this.childDetails[0].clientRect.left;
                }
                renderedSize -= Math.floor(ignoredSize);
                // If there is not enough space we will try to adjust items smaller first.
                var componentClientSize = _this.props.orientation === 1 /* Vertical */ ? componentClientRect.height : componentClientRect.width;
                if (componentClientSize <= renderedSize) {
                    ...
                    while (availableSpace < 0 && _this.hiddenCount < _this.props.responsiveChildren.length) {
                        ...
                        var appliedSize = _this.props.orientation === 1 /* Vertical */ ? childDetail.clientRect.height : childDetail.clientRect.width;
                ...
                // some back to items.
                    ...
                    while (_this.hiddenCount > 0) {
                        ...
                        var childDetail = _this.childDetails[childIndex];
                        // Check if there is enough space for this component.
                        if (_this.childDetails[childIndex].appliedSize >= availableSpace) {
                            break;
                        }
                        // Apply the next adjustment to the child components.
                ...
                if (hiddenCount != _this.hiddenCount) {
                    ...
                    // Now render the updated components and we will see if we have gained enough space.
    ...
    ResponsiveLayout.prototype.render = function () {
        ...
        return React.cloneElement(container, tslib_1.__assign({}, container.props, { ref: this.containerRef }), React.Children.map(container.props.children, function (child, index) {

...
export interface IBoltDragEventDetail<D, S> {
    ...
    /**
     * The native keyboard or mouse event which we are translating into
     * a BoltDragEvent.
     */
    readonly nativeEvent: MouseEvent | KeyboardEvent | TouchEvent;
}
/**
 ...
 */
export declare const enum DragDropEffect {
    /**
 ...
 */
export interface BoltDragEvent<T, D, S> extends React.DragEvent<T> {
    readonly detail: IBoltDragEventDetail<D, S>;
}
export declare function beginDragOperation<D, S>(event: React.MouseEvent<HTMLElement> | React.TouchEvent<HTMLElement>, dataTransfer: IDragDataTransfer<D, S>, minimumPixelsForDrag?: number): IDragDropOperation | undefined;
export declare function dispatchCustomDragEvent<D, S>(eventType: string, target: EventTarget, event: MouseEvent | KeyboardEvent | TouchEvent, dataTransfer: IDragDataTransfer<D, S>): CustomEvent;
export interface IDragItemProps {
    ...
    className?: string;
    /**
     * The drag/drop operation which has the current x/y values for the mouse position
     */
    operation: IDragDropOperation;
    /**
     * Number of pixels to offset the image from the mouse position horizontally.
     * @default 5
     */
    xOffset?: number;
    /**
     * Number of pixels to offset the image from the mouse position vertically.
     * @default 5
     */
    yOffset?: number;
}
